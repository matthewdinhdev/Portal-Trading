...
def format_discord_message(analysis, symbol):
    ...
    color_map = {"bullish": 0x00FF00, "bearish": 0xFF0000, "neutral": 0xFFFF00}  # Green  # Red  # Yellow

    # Create the embed
    embed = {
        "title": f"🔄 Swing Trading Analysis: {symbol}",
        "color": color_map.get(analysis["sentiment"].lower(), 0x808080),
        "timestamp": analysis["timestamp"],
        "fields": [
            {"name": "📊 Market Sentiment", "value": f"```{analysis['sentiment'].upper()}```", "inline": True},
            {"name": "⚠️ Risk Level", "value": f"```{analysis['risk_level'].upper()}```", "inline": True},
            {"name": "🎯 Confidence", "value": f"```{analysis['confidence']}%```", "inline": True},
            {"name": "📈 Recommendation", "value": f"```{analysis['recommendation']}```", "inline": True},
            {
                "name": "💰 Price Targets",
                "value": f"Stop Loss: ${analysis['stop_loss']}\nTake Profit: ${analysis['take_profit']}",
                "inline": True,
            },
            {
                "name": "⏱️ Trade Details",
                "value": f"Holding Period: {analysis['holding_period']}\nRisk:Reward: {analysis['risk_reward_ratio']}\nPosition Size: {analysis['position_size']}",
                "inline": True,
            },
            {
                "name": "🔍 Key Signals",
...
def send_to_discord(analysis, symbol):
    ...
    try:
        ...
        if response.status_code == 204:
            logger.info(f"Successfully sent trading analysis for {symbol} to Discord")
            return True
        else:
